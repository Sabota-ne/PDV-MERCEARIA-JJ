...
if (!CONFIGURADO) {
  ...
  async function carregar() {
    try {
      ...
      var snaps = await Promise.all([
        ...
        db.collection(COL_VENDAS).where("data", ">", corte90).orderBy("data").get(),
        ...
        db.collection(COL_PERDAS).where("data", ">", corte60).orderBy("data").get(),

        // Sangrias — últimos 60 dias
        db.collection(COL_SANGRIAS).where("data", ">", corte60).orderBy("data").get(),

        // Despesas — últimos 60 dias
        db.collection(COL_DESPESAS).where("data", ">", corte60).orderBy("data").get(),
  ...
  function adicionarVenda(v) {
    var obj = clean(v);
    obj.serverTs = firebase.firestore.FieldValue.serverTimestamp();
    return docRef(COL_VENDAS, v.id).set(obj, MERGE).catch(function(e) { console.error("[PDV] adicionarVenda:", e); });
  }
  function adicionarCompra(c)   { return docRef(COL_COMPRAS,  c.id).set(clean(c), MERGE).catch(function(e) { console.error("[PDV] adicionarCompra:", e); }); }
  function adicionarFiado(f) {
    var obj = clean(f);
    obj.serverTs = firebase.firestore.FieldValue.serverTimestamp();
    return docRef(COL_FIADO, f.id).set(obj, MERGE).catch(function(e) { console.error("[PDV] adicionarFiado:", e); });
  }
  function adicionarPerda(p)    { return docRef(COL_PERDAS,   p.id).set(clean(p), MERGE).catch(function(e) { console.error("[PDV] adicionarPerda:", e); }); }
  function adicionarSangria(s)  { return docRef(COL_SANGRIAS, s.id).set(clean(s), MERGE).catch(function(e) { console.error("[PDV] adicionarSangria:", e); }); }
  function adicionarDespesa(d)  { return docRef(COL_DESPESAS, d.id).set(clean(d), MERGE).catch(function(e) { console.error("[PDV] adicionarDespesa:", e); }); }

  function salvarCaixa(c) {
    return docRef(COL_CAIXAS, c.id).set(clean(c), MERGE).catch(function(e) { console.error("[PDV] salvarCaixa:", e); });
  }
  function salvarContaPagar(c) {
    return docRef(COL_CONTAS_PAGAR, c.id).set(clean(c), MERGE).catch(function(e) { console.error("[PDV] salvarContaPagar:", e); });
  }
  function salvarContaReceber(c) {
    return docRef(COL_CONTAS_RECEBER, c.id).set(clean(c), MERGE).catch(function(e) { console.error("[PDV] salvarContaReceber:", e); });
  }

  function quitarFiado(clienteId, todosOsFiados) {
    (todosOsFiados || []).filter(function(f) { return f.clienteId === clienteId; })
      .forEach(function(f) { docRef(COL_FIADO, f.id).set(clean(f), MERGE).catch(function(e) { console.error("[PDV] quitarFiado:", e); }); });
  }
